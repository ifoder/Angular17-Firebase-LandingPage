...
import {
  ...
  PageBlock,
  PageTranslation,
} from '../../models/collections/page.model';
...
export class PagesService extends DocumentTranslationsService {
  ...
  get(id: string) {
    return this.db.getDocument('pages', id).pipe(
      mergeMap(async (page: Page) => {
        const translations = page.translationId
          ? await this.getTranslations(page.translationId)
              .pipe(take(1))
              .toPromise()
          : {};
        page.id = id;
  ...
  getTranslationLanguages(page: Page) {
    ...
    return this.settings
      ...
      .filter((lang: Language) => pageLanguages.indexOf(lang.key) === -1);
  }
    ...
    return pagesObservable.pipe(
      mergeMap(async (pages: Page[]) => {
        ...
        for (let page of pages) {
          // console.log(page);
          page.translations = page.translationId
            ? await this.getTranslations(page.translationId)
                .pipe(take(1))
                .toPromise()
            : {};
          ...
          page.author = page.createdBy
            ...
            : of(null);
          page.isTranslatable = !activeSupportedLanguages.every(
            (lang: string) => pageLanguages.includes(lang)
          );
        }
        //});
        return pages;
  ...
  edit(id: string, data: Page) {
    ...
    return new Promise((resolve, reject) => {
      this.db
        .setDocument('pages', id, page)
        .then(() => {
          // replace blocks
          this.db
            .updateDocument('pages', id, { blocks: data.blocks || {} })
            .then(() => {})
            .catch((error: Error) => {
              reject(error);
            });
  ...
  delete(
    id: string,
    data: { lang: string; translationId: string; translations: PageTranslation }
  ) {
    return new Promise((resolve, reject) => {
      this.deleteTranslation(data.translationId, data.lang, data.translations)
        .then(() => {
          // should be done before deleting document (pages observable will be synced before if not)
          this.db
    ...
    return new Promise((resolve, reject) => {
      this.getWhereFn((ref) =>
        ...
        .then((pages: Page[]) => {
          ...
          resolve(
            pages && pages.length && (!id || (pages[0]['id'] as any) !== id)
          );
        })
        .catch((error: Error) => {
          reject(error);
