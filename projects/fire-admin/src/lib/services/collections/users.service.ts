...
export class UsersService {
  ...
  edit(id: string, data: User, oldData: { email: string; password: string }) {
    ...
    return new Promise((resolve, reject) => {
      this.updateEmail(oldData.email, oldData.password, data.email)
        .then(() => {
          this.updatePassword(data.email, oldData.password, data.password)
            .then(() => {
              this.uploadImageAfter(
                this.db.setDocument('users', id, user),
                user,
  ...
  delete(
    id: string,
    data: { email: string; password: string; avatar: string }
  ) {
    return new Promise((resolve, reject) => {
      this.firebaseUser
        .delete(data.email, data.password)
        .then(() => {
          this.db
