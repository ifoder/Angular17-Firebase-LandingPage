...
import {
  ...
  PostTranslation,
  PostStatus,
} from '../../models/collections/post.model';
...
export class PostsService extends DocumentTranslationsService {
  ...
  constructor(
    protected override db: DatabaseService,
  ...
  add(data: Post, translationId?: string) {
    ...
    return new Promise((resolve, reject) => {
      this.db
        .addDocument('posts', post)
        .then((doc: any) => {
          this.uploadImage(doc.id, data.image as File)
            .then(() => {
              this.addTranslation(data.lang, doc.id, translationId)
    ...
    return new Promise((resolve, reject) => {
      if (imageFile && isFile(imageFile)) {
        ...
        this.storage
          ...
          .then(() => {
            this.db
              .setDocument('posts', id, { image: imagePath })
              .then(() => {
                // resolve();
              })
              .catch((error: Error) => {
  ...
  get(id: string) {
    return this.db.getDocument('posts', id).pipe(
      mergeMap(async (post: Post) => {
        const translations = await this.getTranslations(post.translationId)
          .pipe(take(1))
          .toPromise();
        post.id = id;
    ...
    return postsObservable.pipe(
      mergeMap(async (posts: Post[]) => {
        ...
        for (let post of posts) {
          // console.log(post);
          post.translations = await this.getTranslations(post.translationId)
            .pipe(take(1))
            .toPromise();
          ...
          post.image = {
            ...
            url: post.image
              ? merge(
                  of(getLoadingImage()),
                  this.getImageUrl(post.image as string)
                )
              : of(getEmptyImage()),
          };
          post.author = post.createdBy
            ...
            : of(null);
          post.isTranslatable = !activeSupportedLanguages.every(
            (lang: string) => postLanguages.includes(lang)
          );
        }
        //});
        return posts;
  ...
  edit(id: string, data: Post) {
    ...
    return new Promise((resolve, reject) => {
      this.db
        .setDocument('posts', id, post)
        .then(() => {
          this.uploadImage(id, data.image as File)
            .then(() => {
              // resolve();
            })
            .catch((error: Error) => {
  ...
  async delete(
    ...
    if (data.imagePath) {
      const posts: Post[] = await this.getWhere('image', '==', data.imagePath)
        .pipe(take(1))
        .toPromise();
      if (posts.length > 1) {
    ...
    return new Promise((resolve, reject) => {
      this.deleteTranslation(data.translationId, data.lang, data.translations)
        .then(() => {
          // should be done before deleting document (posts observable will be synced before if not)
          this.db
            .deleteDocument('posts', id)
            .then(() => {
              this.deleteImage(data.imagePath)
                .then(() => {
                  // resolve();
                })
                .catch((error: Error) => {
  ...
  isSlugDuplicated(slug: string, lang: string, id?: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.getWhereFn((ref) =>
        ref.where('slug', '==', slug).where('lang', '==', lang)
      )
        .pipe(take(1))
        .toPromise()
        .then((posts: Post[]) => {
          ...
          resolve(
            posts && posts.length && (!id || (posts[0]['id'] as any) !== id)
          );
        })
        .catch((error: Error) => {
          reject(error);
