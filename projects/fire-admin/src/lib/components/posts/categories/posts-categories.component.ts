...
export class PostsCategoriesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // Get all categories
    this.allCategories = this.categories.getAll().pipe(
      map((categories: Category[]) => {
        return categories.sort(
          (a: Category, b: Category) => b.createdAt - a.createdAt
        );
      })
    );
  ...
  deleteCategory(category: Category) {
    this.categories
      ...
      .then(() => {
        this.alert.success(
          this.i18n.get('CategoryDeleted', { label: category.label }),
          false,
          5000
        );
      })
  ...
  editCategory(category: Category) {
    this.categories
      ...
      .then(() => {
        this.alert.success(
          this.i18n.get('CategorySaved', { label: category.label }),
          false,
          5000
        );
      })
